...
class CRESTJob(Job):
    ...
    def get_conformer_xyz(self, number: int = None):
        ...
        if os.path.exists(self.conformer_directory):
            return [j(self.conformer_directory, file) for i, file in enumerate(os.listdir(self.conformer_directory)) if number and i < number]
    ...
    def get_rotamer_xyz(self, number: int = None):
        ...
        if os.path.exists(self.rotamer_directory):
            return [j(self.rotamer_directory, file) for i, file in enumerate(os.listdir(self.rotamer_directory)) if number and i < number]
