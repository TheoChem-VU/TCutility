...
TPESCoord = Sequence[Sequence[Union[float, int]]]
...
class AMSJob(Job):
    ...
    def transition_state(self, distances: Union[TPESCoord, None] = None, angles: Union[TPESCoord, None] = None, dihedrals: Union[TPESCoord, None] = None, ModeToFollow: int = 1):
        """
    ...
    def IRC(self, direction: str = "both", hess_file: Union[str, None] = None, step_size: float = 0.2, min_path_length: float = 0.1, max_points: int = 300):
        """
        ...
        self.add_postscript(clean_workdir)
    ...
    def electric_field(self, direction: List[float], magnitude: Union[float, None] = None):
        """
        ...
        if magnitude is not None:
            direction = list(np.array(direction) / np.linalg.norm(direction) * magnitude)
