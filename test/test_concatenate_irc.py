...
@pytest.fixture
def forward_irc() -> pl.Path:
    return ircs_dir / "forward"


@pytest.fixture
def forward2_irc() -> pl.Path:
    return ircs_dir / "forward_2"


@pytest.fixture
def backward_irc() -> pl.Path:
    return ircs_dir / "backward"


@pytest.fixture
def backward2_irc() -> pl.Path:
    return ircs_dir / "backward_2"


def test_len_irc_trajectories(forward_irc, forward2_irc, backward_irc, backward2_irc):
    res_objs = [read(str(irc)) for irc in [forward_irc, forward2_irc, backward_irc, backward2_irc]]
...
def test_len_final_trajectory(forward_irc, backward_irc, backward2_irc, forward2_irc):
    ...
    concatenated_mols, _ = concatenate_irc_trajectories([str(irc) for irc in [forward_irc, backward_irc, backward2_irc, forward2_irc]], 10)
    assert (
    ...
    concatenated_mols, _ = concatenate_irc_trajectories([str(irc) for irc in [forward_irc, backward_irc]], 10, reverse=True)
